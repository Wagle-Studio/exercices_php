...
class Db
{
    ...
    public function updateRecordByEmail(string $email, array $updatedData): bool
    {
    ...
    public function deleteRecordByEmail(string $email): bool
    {
        $records = $this->readFromCsv();
        $isDeleted = false;

        $filteredRecords = array_filter($records, function ($row) use ($email, &$isDeleted) {
            if ($row[0] === $email) {
                $isDeleted = true;
                return false;
            }
